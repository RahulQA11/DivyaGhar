...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const productGalleryMainImage = document.getElementById('mainProductImage');
    ...
    if (productGalleryMainImage && thumbnails.length > 0) {
...
function initializeScrollAnimations() {
    ...
    const animateElements = document.querySelectorAll('.features-grid, .categories-grid, .products-grid, .trust-grid');
        ...
        button.addEventListener('click', function(e) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // Update cart count immediately with the new count from response
                    if (data.cart_count !== undefined) {
                        updateCartCountImmediate(data.cart_count);
                    } else {
                        updateCartCount(); // Fallback to fetch count
                    }
 ...
 */
function updateCartCountImmediate(count) {
...
function updateCartCount() {
    fetch('ajax/get_cart_count.php')
        .then(response => response.json())
        .then(data => {
            updateCartCountImmediate(data.count);
        })
